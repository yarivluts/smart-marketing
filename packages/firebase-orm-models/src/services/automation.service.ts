...
import {
  ...
  validateAdCreativeEditActionInput,
...
/** `campaign_draft_create`/`campaign_activation`/`keyword_edit`/`ad_edit`/`meta_ad_set_edit`/`ad_creative_edit` (KAN-72/KAN-73) touch a campaign's full lifecycle, so — unlike `budget_change`, which Optimize already permits — they require the linked connection to be approved at the `manage` tier specifically. */
...
function actionSummaryVerb(actionType: AutomationActionType): string {
  ...
  if (actionType === 'ad_creative_edit') {
...
export interface ProposeAdCreativeEditActionParams {
  organizationId: string;
  projectId: string;
  targetId: string;
 ...
 * ad's creative (primary text/headline/description/link/image) — plan
 * `13 §E21.3`'s own deferred "post-creation edits beyond activation" bullet,
 * the Meta counterpart to `proposeAdEditAction`. `adResourceName` must be one
 * of `target.meta_ad_resource_names` — set by a `campaign_draft_create`
 * action this same target already executed — so a caller can never point an
 * edit at an arbitrary ad outside this target's own campaign (same invariant
 * `proposeAdEditAction`/`proposeMetaAdSetEditAction` establish for their own
 * resource types). Like `meta_ad_set_edit` (and unlike `ad_edit`), the true
 * pre-edit value (the ad's current creative id) isn't known until execute
 * time — `before` only records `adResourceName` here; `executeActionByType`
 * widens it post-execution with the real value `MetaAutomationActionExecutor`
 * reads live.
 */
export async function proposeAdCreativeEditAction(params: ProposeAdCreativeEditActionParams): Promise<AutomationActionModel> {
  await requireProjectInOrg(params.organizationId, params.projectId);
  try {
    validateAdCreativeEditActionInput({ adResourceName: params.adResourceName, creative: params.creative });
  } catch (error) {
    if (error instanceof InvalidCampaignDraftError) {
      throw new InvalidAutomationActionError(error.message);
    }
  ...
  action.project_id = params.projectId;
  action.environment_id = target.environment_id;
  action.action_type = 'ad_creative_edit';
  action.target_id = params.targetId;
  action.target_label = target.label;
  action.before = before;
  action.after = after;
...
async function executeActionByType(
  ...
  if (action.action_type === 'ad_creative_edit') {
    const before = action.before as { adResourceName: string };
    const after = action.after as { adResourceName: string; creative: AdCreativeEditContent };
    const { result, attempts } = await runWithRetryBackoff(
      () =>
    ...
    // Widens both `after` (the real new creative id) and `before` (the ad's real pre-edit
    // creative id, read live) — the same double-widening reasoning `meta_ad_set_edit`
    // documents for its own before-widening, plus `keyword_edit`'s own after-widening,
    // combined: this action type needs both to let `rollbackActionByType` know exactly
    // which creative to point the ad back at.
...
async function rollbackActionByType(
  ...
  if (action.action_type === 'ad_creative_edit') {
    ...
    if (!after.newCreativeResourceName || !before.previousCreativeResourceName) {
      throw new InvalidAutomationActionError('this ad_creative_edit action has no new/previous creative resource name to roll back (it never finished executing)');
    }
    await executor.rollbackAdCreativeEdit({
      organizationId,
      projectId,
      environmentId: action.environment_id,
      targetId: action.target_id,
