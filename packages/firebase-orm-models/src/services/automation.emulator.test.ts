...
describe('proposeAdCreativeEditAction (KAN-73 follow-up)', () => {
  /** Seeds a target and executes a `campaign_draft_create` (Meta platform) against it, so `meta_ad_resource_names` is populated (via the simulated executor, same posture every other creative-edit test in this block relies on). */
  async function seedTargetWithCreatedCampaign(organizationId: string, projectId: string, ownerId: string) {
    const target = await seedTarget(organizationId, projectId, ownerId, 0);
    const created = await proposeCampaignDraftCreateAction({
      organizationId,
  ...
  const NEW_CREATIVE = { primaryText: 'Even bigger savings.', headline: 'Blue Widgets Mega Sale', linkUrl: 'https://example.com/mega-sale' };

  it('proposes a clean ad creative edit as awaiting_approval with the dry-run diff', async () => {
    const { owner, organization, project } = await setupOrgWithProject('Propose Ad Creative Edit Clean Org');
    const target = await seedTargetWithCreatedCampaign(organization.id, project.id, owner.id);
    ...
    expect(action.status).toBe('awaiting_approval');
    expect(action.action_type).toBe('ad_creative_edit');
    expect(action.before).toEqual({ adResourceName });
    expect(action.after).toEqual({ adResourceName, creative: NEW_CREATIVE });
    expect(action.guardrail_violations).toEqual([]);
  });

  it('rejects creative content that fails validation (missing headline)', async () => {
    const { owner, organization, project } = await setupOrgWithProject('Propose Ad Creative Edit Invalid Org');
    const target = await seedTargetWithCreatedCampaign(organization.id, project.id, owner.id);
    const adResourceName = target.meta_ad_resource_names?.[0] as string;
    ...
    await expect(
      proposeAdCreativeEditAction({
        ...
        targetId: target.id,
        adResourceName,
        creative: { ...NEW_CREATIVE, headline: '' },
        requestedByUserId: owner.id,
      }),
    ).rejects.toThrow(InvalidAutomationActionError);
  });
  ...
  it("refuses an ad resource name that is not one of this target's own ads", async () => {
    const { owner, organization, project } = await setupOrgWithProject('Propose Ad Creative Edit Wrong Ad Org');
    const target = await seedTargetWithCreatedCampaign(organization.id, project.id, owner.id);
  ...
  it('refuses a target with no campaign (and so no ads) created yet', async () => {
    const { owner, organization, project } = await setupOrgWithProject('Propose Ad Creative Edit No Campaign Org');
    const target = await seedTarget(organization.id, project.id, owner.id);
  ...
  it('blocks an ad creative edit targeting a protected campaign', async () => {
    const { owner, organization, project } = await setupOrgWithProject('Propose Ad Creative Edit Protected Org');
    const target = await seedTargetWithCreatedCampaign(organization.id, project.id, owner.id);
    const adResourceName = target.meta_ad_resource_names?.[0] as string;
    await setAutomationGuardrailPolicy({
      organizationId: organization.id,
  ...
  it('requires the "manage" write tier specifically — "optimize" is not enough', async () => {
    const { owner, organization, project } = await setupOrgWithProject('Propose Ad Creative Edit Optimize Org');
    const { target, attachment } = await seedTargetWithConnection(organization.id, project.id, owner.id, 'manage', 0);
    const created = await proposeCampaignDraftCreateAction({
      organizationId: organization.id,
      projectId: project.id,
  ...
  it('executes an ad creative edit end to end, widening the diff with the real new/previous creative ids, then rolls it back', async () => {
    const { owner, organization, project } = await setupOrgWithProject('Ad Creative Edit Lifecycle Org');
    const target = await seedTargetWithCreatedCampaign(organization.id, project.id, owner.id);
    const adResourceName = target.meta_ad_resource_names?.[0] as string;

    const proposed = await proposeAdCreativeEditAction({
      organizationId: organization.id,
      projectId: project.id,
      targetId: target.id,
      adResourceName,
    ...
    expect(executed.status).toBe('executed');
    const after = executed.after as { adResourceName: string; creative: unknown; newCreativeResourceName: string };
    const before = executed.before as { adResourceName: string; previousCreativeResourceName: string };
    expect(after.newCreativeResourceName).toEqual(expect.any(String));
    expect(before.previousCreativeResourceName).toEqual(expect.any(String));
