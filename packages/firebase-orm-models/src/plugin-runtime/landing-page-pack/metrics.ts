 ...
 * that, unlike the SaaS/Engagement packs' `fact_funnel_event`/`fact_ad_spend`
 * (plan `04 §1`'s canonical-but-aspirational schema, pending a real
 * warehouse in KAN-18), **actually exists in `@growthos/dbt-transform`** and
 * is built and tested against fixture data on every `pnpm test`. So these
 * three are the first pack metrics whose SQL targets a table the pipeline
 * really produces end to end.
 *
