...
import { CAMPAIGN_OPS_PACK_METRICS } from './metrics';
...
describe('CAMPAIGN_OPS_PACK_MANIFEST_YAML', () => {
  ...
  it('declares registers.metrics as exactly the four metric names this pack registers, regardless of order', () => {
    const manifest = parsePluginManifest(CAMPAIGN_OPS_PACK_MANIFEST_YAML);
    expect(new Set(manifest.registers.metrics)).toEqual(new Set(CAMPAIGN_OPS_PACK_METRICS.map((metric) => metric.name)));
