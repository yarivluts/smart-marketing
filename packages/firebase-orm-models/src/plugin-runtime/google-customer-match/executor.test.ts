...
describe('GoogleCustomerMatchSinkPluginExecutor', () => {
  it('creates a new Customer Match user list on first sync, hashes emails, and reports the list resource name as externalRef', async () => {
    const apiClient = fakeApiClient({ addCustomerMatchUserIdentifiers: vi.fn().mockResolvedValue({ numReceived: 2 }) });
    ...
    expect(apiClient.addCustomerMatchUserIdentifiers).toHaveBeenCalledWith('999', 'customers/999/userLists/new', [
  ...
  it('reuses an already-known user list resource name and never creates a duplicate list', async () => {
    const apiClient = fakeApiClient({ addCustomerMatchUserIdentifiers: vi.fn().mockResolvedValue({ numReceived: 1 }) });
    ...
    expect(apiClient.addCustomerMatchUserIdentifiers).toHaveBeenCalledWith('999', 'customers/999/userLists/existing', [
      { hashedEmail: hashEmailForGoogleCustomerMatch('a@example.com') },
    ]);
  ...
  it('hashes a phone number alongside or instead of email, combining both into one identifier set per record', async () => {
    const apiClient = fakeApiClient({ addCustomerMatchUserIdentifiers: vi.fn().mockResolvedValue({ numReceived: 2 }) });
    const executor = new GoogleCustomerMatchSinkPluginExecutor({
      apiClient,
      customerId: '999',
    ...
    await executor.push(
      pushParams([
        ...
        { properties: { email: 'a@example.com', phone: '5551234567' } },
      ]),
    );

    expect(apiClient.addCustomerMatchUserIdentifiers).toHaveBeenCalledWith('999', 'customers/999/userLists/existing', [
      { hashedPhoneNumber: hashPhoneNumberForGoogleCustomerMatch('+1 (555) 123-4567') },
      { hashedEmail: hashEmailForGoogleCustomerMatch('a@example.com'), hashedPhoneNumber: hashPhoneNumberForGoogleCustomerMatch('5551234567') },
    ]);
  });
  ...
  it('still resolves (creates or reuses) the user list but skips the upload call when no record has a usable email', async () => {
    ...
    expect(apiClient.createCustomerMatchUserList).toHaveBeenCalledWith('999', { name: 'Warm leads' });
    expect(apiClient.addCustomerMatchUserIdentifiers).not.toHaveBeenCalled();
  ...
  it('reuses the user list it just created on a same-instance retry, instead of creating a second one', async () => {
    ...
    // succeeded and only the later addCustomerMatchUserIdentifiers call failed transiently,
    ...
    const apiClient = fakeApiClient({
      addCustomerMatchUserIdentifiers: vi.fn().mockImplementation(() => {
        addEmailsAttempts += 1;
