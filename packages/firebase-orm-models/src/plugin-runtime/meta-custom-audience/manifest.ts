 ...
 * shape exactly. Email-, phone-, mobile-device-id-, and mailing-address-based
 * Custom Audiences are all built here (a record's `properties.email`/
 * `properties.phone`/`properties.device_id`/`properties.first_name`/
 * `properties.last_name`/`properties.city`/`properties.state`/
 * `properties.zip`/`properties.country`, hashed per-connector — see
 * `executor.ts`'s own `extractContactMatchKey`). Lookalike Audience creation from this install's
