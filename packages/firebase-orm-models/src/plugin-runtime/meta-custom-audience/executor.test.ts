...
describe('MetaCustomAudienceSinkPluginExecutor', () => {
  it('creates a new Custom Audience on first sync, hashes emails, and reports the audience id as externalRef', async () => {
    const apiClient = fakeApiClient({ addHashedEmailsToCustomAudience: vi.fn().mockResolvedValue({ numReceived: 2 }) });
    ...
    expect(apiClient.addHashedEmailsToCustomAudience).toHaveBeenCalledWith('audience-new', [
      hashEmailForMetaCustomAudience('a@example.com'),
      hashEmailForMetaCustomAudience('b@example.com'),
    ]);
  ...
  it('reuses an already-known audience id and never creates a duplicate audience', async () => {
    const apiClient = fakeApiClient({ addHashedEmailsToCustomAudience: vi.fn().mockResolvedValue({ numReceived: 1 }) });
    ...
    expect(apiClient.addHashedEmailsToCustomAudience).toHaveBeenCalledWith('audience-existing', [hashEmailForMetaCustomAudience('a@example.com')]);
  ...
  it('still resolves (creates or reuses) the audience but skips the users call when no record has a usable email', async () => {
    ...
    expect(apiClient.createCustomAudience).toHaveBeenCalledWith('999', { name: 'Warm leads' });
    expect(apiClient.addHashedEmailsToCustomAudience).not.toHaveBeenCalled();
  ...
  it('reuses the audience it just created on a same-instance retry, instead of creating a second one', async () => {
    ...
    // and only the later addHashedEmailsToCustomAudience call failed transiently, the retried
    // push() must reuse the audience it already created rather than creating an orphaned duplicate.
    let addEmailsAttempts = 0;
    const apiClient = fakeApiClient({
      addHashedEmailsToCustomAudience: vi.fn().mockImplementation(() => {
        addEmailsAttempts += 1;
