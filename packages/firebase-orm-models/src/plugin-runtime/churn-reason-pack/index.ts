...
import { ensureChurnReasonSchemaRegistered } from '../../services/churn-reason.service';
...
/**
 * Idempotently registers the KAN-84 Churn Reasons pack: the `churn_reason`
 * event schema first (this pack's own metric's `fact_churn_reason` dbt mart
 * is built from it — same schema-before-metrics ordering
 * `ensureFeedbackPackRegistered`/`ensureSaasMetricPackRegistered` establish),
 * then its `churn_events` metric.
 */
export async function ensureChurnReasonPackRegistered(
  ...
  await ensureChurnReasonSchemaRegistered({ organizationId, projectId, createdByUserId });
