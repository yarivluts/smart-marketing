 ...
 * until KAN-18 provisions one — `pipeline/transport.ts`'s `publishPipelineMessage` is the seam a real
 * GCP-backed publish function would slot into without this model changing. Also the first place an
 * accepted record's full raw payload is persisted anywhere durable — KAN-32's own `IngestBatchModel`
 * only ever stored per-record validation status, not the payload.
 */
@Model({
