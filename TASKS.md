...
| KAN-29 | E2.2 KMS envelope encryption for OAuth tokens/secrets (vault module) | 0 | 2 | in-progress | Implemented in `packages/firebase-orm-models`: envelope-encryption primitives (`vault/`), `VaultSecretModel`, `vault.service.ts` (`encryptSecret`/`decryptSecret`/`rotateVaultSecret`). Independently reviewed and tested locally (green `pnpm lint/typecheck/test/build`); PR #14 open against `main`, not yet merged — blocked on the GitHub MCP server needing re-authorization (token expired) to verify CI/merge. See PROGRESS.md. |
