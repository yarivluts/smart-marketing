...
| KAN-26 | E1.6 Hard-isolation & non-enumeration layer: 404-not-403, binding-filtered lists, scoped caches/search/notifications, per-project datasets | 0 | 2 | in-progress | Code complete, self-reviewed, and fully green locally (pnpm lint/typecheck/test/build) on PR #11 (`kan-26-hard-isolation`), scoped to the surfaces that exist today (orgs/projects/members/invites in apps/web — caches/search/notifications/per-project datasets don't exist yet per docs/plan/08-generic-platform.md §5.6, so those bullets are forward-looking for later epics). `requireOrgPermission` now returns 404 (not 403) when the caller has no active membership in an org; 403 is reserved for a real member missing a specific permission. Added a cross-org isolation test suite and a filesystem-scanning guard test. **Not yet merged**: PR #11's CI stalled on the Test step 3/3 attempts (see PROGRESS.md) — waiting on a human decision to merge without a green check or investigate the CI environment. apps/api's `PermissionGuard` still returns a single 403 for both cases; flagged as a follow-up once apps/api gains real org-scoped endpoints. |
