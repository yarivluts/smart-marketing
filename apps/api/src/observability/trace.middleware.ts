...
import { generateTraceId, runWithTraceId } from '@growthos/shared/observability';
...
/** Matches `generateTraceId()`'s own 32-hex-char shape, so an inbound `x-request-id` can only
 * ever become the trace id if it already looks like one — never an arbitrary client string. */
const TRACE_ID_SHAPE = /^[0-9a-f]{32}$/i;
...
export class TraceMiddleware implements NestMiddleware {
  use(req: IncomingRequestLike, res: OutgoingResponseLike, next: () => void): void {
    ...
    const traceId = inbound && TRACE_ID_SHAPE.test(inbound) ? inbound : generateTraceId();
