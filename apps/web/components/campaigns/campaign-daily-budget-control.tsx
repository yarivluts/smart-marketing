...
import { Edit2, Check, X, Loader2, AlertCircle, Plus, Minus } from 'lucide-react';
...
export function CampaignDailyBudgetControl({
  ...
  function handleStep(delta: number): void {
    const nextVal = Math.max(1, budget + delta);
    executeBudgetUpdate(nextVal);
  }
          ...
          <div className="flex items-center gap-1">
            ...
            {!disabled && !isSyncing && (
              <button
                ...
                data-testid={`budget-minus-${targetId}`}
                className="flex h-6 w-6 items-center justify-center rounded-md border border-border bg-card text-muted-foreground hover:bg-muted hover:text-foreground transition-colors cursor-pointer"
                title="- $25"
              >
            ...
            {/* Budget Display Button */}
            <button
              type="button"
              disabled={disabled || isSyncing}
              onClick={() => setIsEditing(true)}
              className="group inline-flex items-center gap-1.5 rounded-lg border border-transparent px-2 py-1 text-xs font-bold text-foreground hover:border-border hover:bg-muted/50 transition-all disabled:cursor-not-allowed disabled:opacity-50 cursor-pointer"
            >
              <span dir="ltr">{`$${budget}/day`}</span>
              {!disabled && !isSyncing && (
            ...
            {!disabled && !isSyncing && (
              <button
                type="button"
                onClick={() => handleStep(25)}
                data-testid={`budget-plus-${targetId}`}
                className="flex h-6 w-6 items-center justify-center rounded-md border border-border bg-card text-muted-foreground hover:bg-muted hover:text-foreground transition-colors cursor-pointer"
                title="+ $25"
              >
            ...
            {isSyncing && (
              <Loader2 className="h-3.5 w-3.5 animate-spin text-muted-foreground ms-1" aria-hidden="true" />
            )}
          </div>
        )}
        ...
        {!disabled && !isEditing && !isSyncing && (
          <div className="flex items-center gap-1 ms-1">
            <button
              ...
              className="rounded-md bg-muted/80 px-1.5 py-0.5 text-[10px] font-semibold text-muted-foreground hover:bg-emerald-100 hover:text-emerald-800 dark:hover:bg-emerald-950 dark:hover:text-emerald-300 transition-colors cursor-pointer"
            ...
            <button
              type="button"
              onClick={() => handlePreset(-20)}
              className="rounded-md bg-muted/80 px-1.5 py-0.5 text-[10px] font-semibold text-muted-foreground hover:bg-rose-100 hover:text-rose-800 dark:hover:bg-rose-950 dark:hover:text-rose-300 transition-colors cursor-pointer"
