...
import { POST as proposeAdCreativeEditRoute } from '@/app/api/orgs/[orgId]/projects/[projectId]/automation/actions/ad-creative-edits/route';
...
describe('org-scoped route isolation across two real orgs (KAN-26 non-enumeration)', () => {
  ...
  it('POST /api/orgs/[orgId]/projects/[projectId]/automation/actions/ad-creative-edits: org caller cannot see vs. fake org id (KAN-73 follow-up)', async () => {
    const callerSession = await sessionFor(unique('uid'), uniqueEmail('iso-ad-creative-edit-caller'));
    const caller = await ensureUserForFirebaseSession({ firebaseUid: callerSession.uid, email: callerSession.email as string });
    await createOrganizationWithOwner({ name: 'Isolation Org A (ad creative edit)', ownerUserId: caller.id });

    const otherOwner = await ensureUserForFirebaseSession({ firebaseUid: unique('uid'), email: uniqueEmail('iso-ad-creative-edit-b-owner') });
    const { organization: orgB } = await createOrganizationWithOwner({ name: 'Isolation Org B (ad creative edit)', ownerUserId: otherOwner.id });

    getServerSessionMock.mockResolvedValue(callerSession);

    const postRequestFor = (orgId: string, projectId: string) =>
      new NextRequest(`https://growthos.test/api/orgs/${orgId}/projects/${projectId}/automation/actions/ad-creative-edits`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    await expectIndistinguishable(
      () =>
        proposeAdCreativeEditRoute(postRequestFor(orgB.id, FAKE_ORG_ID), { params: Promise.resolve({ orgId: orgB.id, projectId: FAKE_ORG_ID }) }),
      () =>
        proposeAdCreativeEditRoute(postRequestFor(FAKE_ORG_ID, FAKE_ORG_ID), {
          params: Promise.resolve({ orgId: FAKE_ORG_ID, projectId: FAKE_ORG_ID }),
        }),
    );
  });
