 ...
 * each to gate on `requireOrgPermission`, `requireProjectPermission`
 * (KAN-136 — the project-scoped sibling that also satisfies a project-scope
 * role binding), or `requireOrgMembership` (404-not-403 non-enumeration, see
 * access.ts — all three share the same membership-check core,
 * `requireOrgPermission`/`requireProjectPermission` just add a `can()` check
 * on top for routes that need one) or be explicitly listed below with a
 * reason it's safe without it. This repo has no per-route annotation system
 * for apps/web the way apps/api's `@RequirePermission` decorator does (see
 * the `growthos/require-permission-annotation` eslint rule there), so this
 * test is the equivalent guardrail: it fails CI the moment a new org-scoped
 * route file appears with neither gate call nor an entry here.
 *
 * Add a new org-scoped route -> either call `requireOrgPermission`,
 * `requireProjectPermission`, or `requireOrgMembership` in it (no change
 * needed here), or add a one-line justified exemption below — and either
 * way, add a matching scenario to `isolation.test.ts` asserting it returns
 * identical responses for "real org I can't see" vs. "fake org id".
 */
  ...
  it('every route.ts under app/api/orgs and app/api/invites gates on requireOrgPermission or is a justified exemption', () => {
    ...
    const unguarded = files
      ...
      .filter((relative, index) => {
        ...
        const isGated =
          source.includes('requireOrgPermission') ||
          source.includes('requireProjectPermission') ||
          source.includes('requireOrgMembership');
    ...
    expect(
      unguarded,
      `Found route(s) with no requireOrgPermission/requireProjectPermission/requireOrgMembership gate and no ` +
        `EXEMPT_ROUTES entry: ${unguarded.join(', ')}. Wire in one of those, or add a justified exemption to ` +
        'route-isolation-guard.test.ts, and add an isolation.test.ts scenario for it.',
    ).toEqual([]);
  });
