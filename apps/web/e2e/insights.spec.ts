...
test.describe('Insights (KAN-114)', () => {
  test('an org owner reaches Insights via nav and sees the empty state for a fresh project', async ({ page }) => {
    ...
    await page.getByLabel('Project name').fill('Client Insighton');
    ...
    await expect(page.getByRole('heading', { name: 'Insights for Client Insighton' })).toBeVisible();
    await expect(page.getByText('Nothing noteworthy yet — no active tracking alerts and no wins recorded.')).toBeVisible();
  });
});
