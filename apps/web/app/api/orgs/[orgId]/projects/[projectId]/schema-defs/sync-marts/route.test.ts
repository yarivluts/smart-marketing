...
describe('POST /api/orgs/[orgId]/projects/[projectId]/schema-defs/sync-marts', () => {
  ...
  it('KAN-140: lets a project-scoped project_admin sync marts for THEIR OWN project', async () => {
    const { organization, project, owner } = await setupOrgProject('Sync Marts Project-Scoped Org');
  ...
  it(
    ...
    async () => {
      const { organization, project, owner } = await setupOrgProject('Sync Marts Sibling Project Org');
      const { project: otherProject } = await createProject({ organizationId: organization.id, name: 'Other Project' });
      const memberSession = await inviteProjectScopedMember(organization.id, project.id, 'project_admin', owner.id);

      getServerSessionMock.mockResolvedValue(memberSession);
      const { request, params } = syncMartsRequest(organization.id, otherProject.id);
      const response = await POST(request, { params });
      expect(response.status).toBe(403);
    },
  );
