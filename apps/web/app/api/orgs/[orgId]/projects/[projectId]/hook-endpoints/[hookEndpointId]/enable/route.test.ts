...
import { createHookEndpoint, createOrganizationWithOwner, createProject, disableHookEndpoint, ensureUserForFirebaseSession } from '@growthos/firebase-orm-models';
...
async function setupDisabledEndpoint(orgName: string) {
  ...
  await disableHookEndpoint({ organizationId: organization.id, projectId: project.id, hookEndpointId: endpoint.id, disabledByUserId: owner.id });
  return { ownerSession, organization, project, endpoint };
}

describe('POST /api/orgs/[orgId]/projects/[projectId]/hook-endpoints/[hookEndpointId]/enable', () => {
  ...
  it('re-enables a disabled endpoint', async () => {
    const { ownerSession, organization, project, endpoint } = await setupDisabledEndpoint('Hooks Enable Org');
  ...
  it('returns 404 for an endpoint that does not exist in this project', async () => {
    const { ownerSession, organization, project } = await setupDisabledEndpoint('Hooks Enable Missing Org');
    getServerSessionMock.mockResolvedValue(ownerSession);
