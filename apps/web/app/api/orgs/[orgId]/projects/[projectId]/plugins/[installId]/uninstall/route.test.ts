...
describe('POST /api/orgs/[orgId]/projects/[projectId]/plugins/[installId]/uninstall', () => {
  ...
  it('uninstalls an active install', async () => {
    const { ownerSession, organization, project, install } = await setupInstalledPlugin('Plugin Uninstall Route Org');
    ...
    expect(body.install.status).toBe('uninstalled');
  });

  it('returns 409 when uninstalling an install that is already uninstalled', async () => {
    const { ownerSession, organization, project, install } = await setupInstalledPlugin('Plugin Uninstall Route Twice Org');
    getServerSessionMock.mockResolvedValue(ownerSession);
