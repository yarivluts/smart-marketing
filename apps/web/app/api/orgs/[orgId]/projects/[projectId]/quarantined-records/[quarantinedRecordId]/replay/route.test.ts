...
function replayRequest(
  ...
  return {
    request: new NextRequest(
      `https://growthos.test/api/orgs/${orgId}/projects/${projectId}/quarantined-records/${quarantinedRecordId}/replay`,
      { method: 'POST' },
    ),
...
describe('POST /api/orgs/[orgId]/projects/[projectId]/quarantined-records/[quarantinedRecordId]/replay', () => {
  it('rejects an unauthenticated caller', async () => {
    getServerSessionMock.mockResolvedValue(null);
    const { request, params } = replayRequest('org-1', 'project-1', 'record-1');
  ...
  it('returns 404 for a caller with no active membership in the org', async () => {
    ...
    const { request, params } = replayRequest('does-not-exist-org', 'does-not-exist-project', 'record-1');
    const response = await POST(request, { params });
  ...
  it("rejects a member whose role doesn't hold ingest.write (viewer)", async () => {
    ...
    const { request, params } = replayRequest(organization.id, project.id, 'record-1');
    const response = await POST(request, { params });
    expect(response.status).toBe(403);
  });

  it('returns 404 for a quarantined record id that does not exist in this project', async () => {
    const { ownerSession, organization, project } = await setupOrgProject('Replay Route Missing Record Org');
    getServerSessionMock.mockResolvedValue(ownerSession);
    const { request, params } = replayRequest(organization.id, project.id, 'does-not-exist');
    const response = await POST(request, { params });
    expect(response.status).toBe(404);
  ...
  it("returns 404 for a quarantined record id that belongs to a sibling org's project", async () => {
    const { owner, organization, project, prodEnvironment } = await setupOrgProject('Replay Route Isolation Org A');
    const other = await setupOrgProject('Replay Route Isolation Org B');
    ...
    const { request, params } = replayRequest(other.organization.id, other.project.id, recordId);
    const response = await POST(request, { params });
    expect(response.status).toBe(404);
  });

  it('replays a quarantined record and returns the accepted outcome', async () => {
    const { ownerSession, owner, organization, project, prodEnvironment } = await setupOrgProject('Replay Route Success Org');
    const recordId = await quarantineOneRecord(organization.id, project.id, prodEnvironment.id, owner.id);

    // Schema fix: evolve to register the field that got this record quarantined.
    ...
    getServerSessionMock.mockResolvedValue(ownerSession);
    const { request, params } = replayRequest(organization.id, project.id, recordId);
    const response = await POST(request, { params });
  ...
  it('reports still_quarantined with refreshed reasons when the schema fix has not landed yet', async () => {
    const { ownerSession, owner, organization, project, prodEnvironment } = await setupOrgProject('Replay Route Still Bad Org');
    const recordId = await quarantineOneRecord(organization.id, project.id, prodEnvironment.id, owner.id);
    ...
    const response = await POST(request, { params });
    expect(response.status).toBe(200);
    expect(await response.json()).toEqual({ outcome: 'still_quarantined', reasons: ['unregistered_field:referrer'] });

    const [stillThere] = await listQuarantinedRecordsForProject(organization.id, project.id);
    expect(stillThere.id).toBe(recordId);
    expect(stillThere.status).toBe('quarantined');
  });

  it('KAN-141: lets a project-scoped project_admin replay a quarantined record in THEIR OWN project', async () => {
    const { owner, organization, project, prodEnvironment } = await setupOrgProject('Replay Route Project-Scoped Org');
    const recordId = await quarantineOneRecord(organization.id, project.id, prodEnvironment.id, owner.id);
  ...
  it(
    ...
    async () => {
      const { organization, project, owner } = await setupOrgProject('Replay Route Sibling Project Org');
      const { project: otherProject } = await createProject({ organizationId: organization.id, name: 'Other Project' });
      const memberSession = await inviteProjectScopedMember(organization.id, project.id, 'project_admin', owner.id);

      getServerSessionMock.mockResolvedValue(memberSession);
      const { request, params } = replayRequest(organization.id, otherProject.id, 'record-1');
      const response = await POST(request, { params });
      expect(response.status).toBe(403);
    },
  );
