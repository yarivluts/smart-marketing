...
import { uninstallPlugin } from '@/lib/orgs/mutations';
...
/** Terminally removes an install (KAN-46 AC: "uninstall" lifecycle) — gated on `plugin.install`, same as installing. */
export async function POST(_request: Request, { params }: RouteParams): Promise<NextResponse> {
  ...
  try {
    const install = await uninstallPlugin({ organizationId: orgId, projectId, installId, performedByUserId: user.id });
