...
/** Seeds a target and executes a `campaign_draft_create` against it, so `ad_resource_names` is populated. */
...
function adEditsUrl(orgId: string, projectId: string): string {
  return `https://growthos.test/api/orgs/${orgId}/projects/${projectId}/automation/actions/ad-edits`;
}
...
function postRequest(orgId: string, projectId: string, body: string): { request: NextRequest; params: Promise<{ orgId: string; projectId: string }> } {
  return {
    request: new NextRequest(adEditsUrl(orgId, projectId), {
      method: 'POST',
...
describe('POST /api/orgs/[orgId]/projects/[projectId]/automation/actions/ad-edits', () => {
  it('rejects an unauthenticated caller', async () => {
    getServerSessionMock.mockResolvedValue(null);
    const { request, params } = postRequest(
      'org-1',
      'project-1',
      JSON.stringify({ targetId: 'x', previousAdResourceName: 'customers/1/adGroupAds/1', responsiveSearchAd: RESPONSIVE_SEARCH_AD }),
    );
  ...
  it("rejects a member whose role doesn't hold automation.execute (viewer)", async () => {
    const { owner, organization } = await setupOrgWithProject('Ad Edits POST Viewer Org');
  ...
  it('returns 400 invalid_json for a malformed body', async () => {
    const { ownerSession, organization, project } = await setupOrgWithProject('Ad Edits POST Bad JSON Org');
  ...
  it('returns 400 target_id_required when targetId is missing or blank', async () => {
    const { ownerSession, organization, project } = await setupOrgWithProject('Ad Edits POST Missing Target Org');
    getServerSessionMock.mockResolvedValue(ownerSession);

    const missing = postRequest(organization.id, project.id, JSON.stringify({ previousAdResourceName: 'customers/1/adGroupAds/1', responsiveSearchAd: RESPONSIVE_SEARCH_AD }));
    expect((await POST(missing.request, { params: missing.params })).status).toBe(400);

    const blank = postRequest(
      organization.id,
      project.id,
      JSON.stringify({ targetId: '   ', previousAdResourceName: 'customers/1/adGroupAds/1', responsiveSearchAd: RESPONSIVE_SEARCH_AD }),
    );
  ...
  it('returns 400 previous_ad_resource_name_required when it is missing or blank', async () => {
    const { ownerSession, organization, project } = await setupOrgWithProject('Ad Edits POST Missing Ad Org');
    getServerSessionMock.mockResolvedValue(ownerSession);

    const missing = postRequest(organization.id, project.id, JSON.stringify({ targetId: 'campaign-1', responsiveSearchAd: RESPONSIVE_SEARCH_AD }));
    expect((await POST(missing.request, { params: missing.params })).status).toBe(400);

    const blank = postRequest(organization.id, project.id, JSON.stringify({ targetId: 'campaign-1', previousAdResourceName: '  ', responsiveSearchAd: RESPONSIVE_SEARCH_AD }));
    const blankResponse = await POST(blank.request, { params: blank.params });
    expect(blankResponse.status).toBe(400);
    expect(await blankResponse.json()).toEqual({ error: 'previous_ad_resource_name_required' });
  });
  ...
  it('returns 404 not_found for a target id that does not exist in the project', async () => {
    ...
    const { request, params } = postRequest(
      organization.id,
      project.id,
      JSON.stringify({ targetId: 'does-not-exist', previousAdResourceName: 'customers/1/adGroupAds/1', responsiveSearchAd: RESPONSIVE_SEARCH_AD }),
    );
    const response = await POST(request, { params });
  ...
  it('returns 400 invalid_ad_edit for an ad that is not one of this target\'s own ads', async () => {
    const { ownerSession, owner, organization, project } = await setupOrgWithProject('Ad Edits POST Wrong Ad Org');
    ...
    const { request, params } = postRequest(
      organization.id,
      project.id,
      JSON.stringify({ targetId: target.id, previousAdResourceName: 'customers/999/adGroupAds/not-this-targets', responsiveSearchAd: RESPONSIVE_SEARCH_AD }),
    );
    const response = await POST(request, { params });
    expect(response.status).toBe(400);
    const body = (await response.json()) as { error: string; message: string };
    expect(body.error).toBe('invalid_ad_edit');
    expect(body.message).toContain('own ads');
  });

  it('returns 400 invalid_ad_edit for an invalid RSA (too few headlines)', async () => {
    const { ownerSession, owner, organization, project } = await setupOrgWithProject('Ad Edits POST Invalid RSA Org');
    const target = await seedTargetWithCreatedCampaign(organization.id, project.id, owner.id);
    getServerSessionMock.mockResolvedValue(ownerSession);
    ...
    const { request, params } = postRequest(
      ...
      JSON.stringify({
        targetId: target.id,
        previousAdResourceName: target.ad_resource_names?.[0],
        responsiveSearchAd: { ...RESPONSIVE_SEARCH_AD, headlines: ['Only One'] },
      }),
    );
    const response = await POST(request, { params });
    expect(response.status).toBe(400);
    expect((await response.json() as { error: string }).error).toBe('invalid_ad_edit');
  });

  it('proposes a clean ad edit and lands it awaiting_approval with no guardrail violations', async () => {
    const { ownerSession, owner, organization, project } = await setupOrgWithProject('Ad Edits POST Clean Org');
    const target = await seedTargetWithCreatedCampaign(organization.id, project.id, owner.id);
    getServerSessionMock.mockResolvedValue(ownerSession);
    ...
    const { request, params } = postRequest(
      ...
      JSON.stringify({
        targetId: target.id,
        previousAdResourceName: target.ad_resource_names?.[0],
        responsiveSearchAd: RESPONSIVE_SEARCH_AD,
      }),
    );
    const response = await POST(request, { params });
