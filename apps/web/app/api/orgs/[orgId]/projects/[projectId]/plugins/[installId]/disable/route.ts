...
import { disablePlugin } from '@/lib/orgs/mutations';
...
/** Pauses an active install without losing its config/consent (KAN-46 AC: "disable" lifecycle) — gated on `plugin.install`, same as installing. */
export async function POST(_request: Request, { params }: RouteParams): Promise<NextResponse> {
  ...
  try {
    const install = await disablePlugin({ organizationId: orgId, projectId, installId, performedByUserId: user.id });
