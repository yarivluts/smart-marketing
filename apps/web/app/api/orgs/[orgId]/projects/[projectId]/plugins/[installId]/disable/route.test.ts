...
describe('POST /api/orgs/[orgId]/projects/[projectId]/plugins/[installId]/disable', () => {
  ...
  it('disables an active install', async () => {
    const { ownerSession, organization, project, install } = await setupInstalledPlugin('Plugin Disable Route Org');
    ...
    expect(body.install.status).toBe('disabled');
  });

  it('returns 409 when disabling an install that is not currently installed', async () => {
    const { ownerSession, organization, project, install } = await setupInstalledPlugin('Plugin Disable Route Twice Org');
    getServerSessionMock.mockResolvedValue(ownerSession);
  ...
  it('returns 404 for an install id that does not exist', async () => {
    const { ownerSession, organization, project } = await setupInstalledPlugin('Plugin Disable Route Missing Org');
    getServerSessionMock.mockResolvedValue(ownerSession);
    const response = await POST(new Request('https://growthos.test'), {
